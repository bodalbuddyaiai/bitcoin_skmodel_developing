...
async def set_ai_model(request: Request):
    ...
    try:
        ...
        # 모델 타입 검증 - claude-opus 추가
        if model_type.lower() not in ["gpt", "openai", "claude", "anthropic", "claude-opus", "opus"]:
            raise HTTPException(status_code=400, detail="지원하지 않는 모델 타입입니다. (gpt, claude, claude-opus만 지원)")
...
async def get_ai_model():
    ...
    try:
        ...
        return {
            ...
            "available_models": ["gpt", "claude", "claude-opus"]
        }
