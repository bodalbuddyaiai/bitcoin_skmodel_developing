...
**변동성 기반 의사결정 필수 사항:**
1. ATR %를 먼저 계산하여 현재 변동성 레짐을 분류하세요
2. 볼린저 밴드 폭(표준편차)도 변동성 레짐 판단에 활용하세요
3. 변동성 레짐에 맞는 손절/익절 배수와 최소 손익비를 적용하세요
4. 피보나치 레벨, 피벗 포인트, 스윙 고점/저점으로 주요 지지/저항선을 파악하세요
5. 익절 목표가 저항선(롱)/지지선(숏) ±1% 이내를 관통하면 반드시 조정하세요
6. 조정 후에도 최소 손익비가 충족되는지 확인하세요
