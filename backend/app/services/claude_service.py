...
**빗각 그리는 방법:**
1. **역사적 저점 찾기**: 주어진 캔들봉 데이터 내에서 가장 낮은 가격을 형성한 역사적 저점 식별
2. **마지막 저점 찾기**: 전고점에서 하락하는 과정에서 형성된 중간 지점의 저점 (변곡점) 식별
   - 전고점에서 떨어지면서 중간 정도에 만들어진 저점
   - 최근에 형성된 저점이지만 역사적 저점은 아님
3. **빗각 연결**: 역사적 저점과 마지막 저점 두 점을 이은 대각선(추세선)이 빗각

**빗각 기반 진입 전략:**
- **롱(LONG) 진입 조건**:
  1. 가격이 빗각을 한번 뚫고 올라감 (브레이크아웃 발생)
  2. 다시 내려와서 빗각 선에 닿았을 때 지지받고 반등 (리테스트)
  3. → 빗각이 지지선으로 확인되면 롱 진입
  
- **숏(SHORT) 진입 조건**:
  1. 가격이 빗각을 뚫지 못하고 아래로 떨어짐
  2. 빗각이 저항선으로 작용
  3. → 빗각 돌파 실패 시 숏 진입

**빗각 분석 시 고려사항:**
- 각 시간대별(1분, 5분, 15분, 1시간, 4시간) 캔들봉에서 빗각을 그려볼 것
- 15분 차트의 빗각에 60% 가중치, 1시간 차트 빗각에 25% 가중치
- 빗각 돌파 후 리테스트는 매우 강력한 진입 신호
- 빗각을 활용하여 STOP_LOSS_ROE와 TAKE_PROFIT_ROE 설정 가능

### 핵심 진입 조건:
- 진입 조건을 더 많이 충족하는 방향으로 포지션 진입할 것

**롱 포지션 진입 조건(아래 6가지 진입 조건 중 최소 2개 조건 이상 동시 충족 시 반드시 진입):**
...
6. **빗각 확인**: 빗각을 돌파 후 리테스트하여 지지선으로 작동

**숏 포지션 진입 조건(아래 6가지 진입 조건 중 최소 2개 조건 이상 동시 충족 시 반드시 진입):**
...
## ANALYSIS_DETAILS
**Step 1: 빗각 분석 (각 시간대별)**
[15분, 1시간 차트에서 역사적 저점과 마지막 저점 식별, 빗각 그리기, 현재 가격과 빗각의 관계 분석]
...
**Step 5: 진입 조건 체크**
[위 6개 조건 중 몇 개 충족하는지 구체적으로 확인, 특히 빗각 조건 여부]
...
**Step 1: 빗각 분석 (Diagonal Line Analysis)**
1. **15분 차트 빗각** (60% 가중치):
   - 캔들스틱 데이터에서 역사적 저점 찾기 (최저가)
   - 전고점에서 하락 중 형성된 마지막 저점(변곡점) 찾기
   - 두 점을 연결한 빗각(대각선 추세선) 그리기
   - 현재 가격이 빗각 대비 어느 위치인지 파악
...
3. **빗각 기반 진입 신호**:
   - 롱: 빗각 돌파 후 리테스트하여 지지받는지 확인
   - 숏: 빗각을 돌파하지 못하고 저항받는지 확인
...
**Step 3: 진입 조건 체크 (롱/숏 각각 6개 조건)**

**롱 진입 조건** - 아래 6개 중 **최소 2개 이상 충족 시 진입**:
✓ 빗각을 돌파 후 리테스트하여 지지선으로 작동
...
**숏 진입 조건** - 아래 6개 중 **최소 2개 이상 충족 시 진입**:
✓ 빗각 돌파 실패하여 저항선으로 작동
...
→ **2개 이상 충족하면 적극 진입, 4-6개 충족하면 매우 강한 신호**
→ **빗각 조건이 충족되면 진입 신뢰도가 크게 증가**
→ **추가로 ADX ≥ 20이면 신뢰도 증가, 다중 시간대 일관성 ≥ 60점이면 더욱 유리**

**Step 4: 손익 목표 설정**
1. 빗각과 지지/저항선을 함께 활용하여 take_profit_roe, stop_loss_roe 설정
