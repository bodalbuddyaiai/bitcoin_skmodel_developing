...
**추세 성숙도 판단 (일봉, 4시간봉 기준):**

**중요**: 성숙도는 **상위 시간대(일봉, 4시간봉)**에서만 판단합니다.
1시간봉 4개 하락 ≠ 하락 추세, 상위 추세 내의 조정/반등일 수 있음.

A) **신생 추세** (일봉/4시간봉 기준 최근 1-2일 내 형성)
   - 특징: 일봉/4시간봉 EMA가 최근에 교차, 추세 전환된 지 얼마 안 됨
   - 기대: 추세가 한동안 지속될 가능성 높음
   - 익절 전략: 멀리 설정 (ATR × 4-6)
   
B) **성숙 추세** (일봉/4시간봉 기준 2-5일 지속)
   - 특징: 추세가 한동안 유지됨, 이동평균선 간격이 넓음
   - 기대: 추세가 곧 전환될 수 있음
   - 익절 전략: 적당히 설정 (ATR × 2.5-4)
   
C) **과성숙 추세** (일봉/4시간봉 기준 5일 이상 지속)
   - 특징: 장시간 같은 방향, 일봉/4시간봉 RSI가 극단적
   - 기대: 조정 또는 반전 임박
   - 익절 전략: 가깝게 설정 (ATR × 1.5-2.5) 또는 **진입 보류**

**🚫 과열 구간 진입 금지 (매우 중요):**
- 1시간봉 기준 **최근 4-6개 봉 동안 2% 이상 급격한 변동**이 있었다면:
  * 급락 후(RSI < 30) → **숏 진입 금지**, 반등 후 상위 추세 방향 진입 대기
  * 급등 후(RSI > 70) → **롱 진입 금지**, 조정 후 상위 추세 방향 진입 대기
...
**3. 추세 성숙도 분석 (일봉, 4시간봉 기준만):**
- 일봉/4시간 추세 형성 시점: [N일 전]
- 성숙도: [신생/성숙/과성숙]
- 이동평균선 간격: [넓음/보통/좁음]
- 최근 크로스 이벤트: [있음/없음]
